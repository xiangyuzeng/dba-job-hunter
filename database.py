...
def get_jobs(status=None, source=None, favorite_only=False, search=None,
    ...
    _sql_exclude = [
        "%frontend%", "%react developer%", "%angular%", "%vue.js%",
        "%full stack%", "%fullstack%", "%ios developer%", "%android developer%",
        "%ui/ux%", "%uiux%", "%mobile dev%", "%web developer%",
        "%java developer%", "%ruby%", "%php developer%", "%.net developer%",
        "%scrum master%", "%product manager%", "%project manager%",
        "%sales engineer%", "%marketing%", "%recruiter%", "%graphic design%",
        "%machine learning%", "%ml engineer%", "%ai engineer%",
        "%penetration test%", "%help desk%", "%network engineer%",
        "%cybersecurity%", "%security analyst%", "%qa automation%",
        "%qa engineer%", "%test engineer%", "%blockchain%",
    ]
    for pattern in _sql_exclude:
        conditions.append("LOWER(title) NOT LIKE ?")
        params.append(pattern)

    # Only show jobs with titles relevant to DBA/Data/SRE/Support/Infra
    _relevant_keywords = [
        "database", "data", "dba", "support", "sre", "reliability",
        "infrastructure", "devops", "platform", "sql", "postgres",
        "oracle", "mysql", "mongo", "redis", "snowflake", "admin",
        "etl", "warehouse", "pipeline", "airflow", "spark", "hadoop",
        "bigquery", "redshift", "databricks", "terraform", "kubernetes",
        "k8s", "docker", "linux", "systems", "cloud", "aws", "azure",
        "gcp", "operations", "storage", "backup", "engineer",
    ]
